    ...
    public class ApplicationBehaviour : IBehaviour
    {
        ...
        public void Init(MainWindow window)
        {
            ...
            catch (Exception ex)
            {
                MessageBox.Show(window,
                    "An error occured: " + ex.Message+ " (Try running AmadeusAI as an administrator) ");
            }
        }
        ...
        private void PopulateResponseTable(List<amareresponse> characterResponses)
        {
            foreach (var response in characterResponses)
            {
                ...
                // Opera isn't supported yet
                for (var i = 0; i < triggers.Length; i++)
                {
                    // If trigger is a browser, only respond if the user recently launched the browser
                    if (triggers[i].Contains("firefox") || triggers[i].Contains("chrome") || triggers[i].Contains("Msedge") ||
